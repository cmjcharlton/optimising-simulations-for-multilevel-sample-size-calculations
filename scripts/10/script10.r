...
# Figure 12a
cutoff <- 0.8
figure12a <- ggplot(data = output, aes(x = N)) +
...
# Figure 12b
cutoff <- qnorm(0.8)
figure12b <- ggplot(data = output, aes(x = sqrtN)) +
  geom_point(aes(y = zscore, colour = "SE")) +
  geom_smooth(method = "loess",
              aes(y = zscore, colour = "SE"),
              se = FALSE) +
  geom_point(aes(y = zscore2, colour = "0/1")) +
  geom_smooth(method = "loess",
              aes(y = zscore2, colour = "0/1"),
              se = FALSE) +
  geom_line(y = cutoff, colour = "grey25") +
  scale_colour_manual(name = "method", values = c("black", "grey75")) +
...
figure12 <- figure12a + figure12b + plot_layout(guide = "collect") & theme(legend.position = "bottom")
ggsave("figure12.svg", plot=figure12, width=16, height=10, units="cm")